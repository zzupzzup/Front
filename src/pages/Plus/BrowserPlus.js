...
const BrowserPlus = ()=>{
  ...
  return(
    <div>
      <Header></Header>
      <div className="plus">
        <div className="plus-title"><span>{age}0</span>대 <span>{gender}</span>들이 많이 간 식당</div>
        <div className="plus-modal">
          <div className='type' style={{justifyContent:"flex-start"}}>
            <div><img src={redArea} className="plus-area-img"/></div>
            {areas.map((item, index) => (
              <button key={index} className={area.includes(item) ? 'types yellow' : 'types'} onClick={() => handleAreaClick(item)}>
                {item}
              </button>
            ))}
            <div style={{margin:"13px"}}>
              <button className={allAreaCheck ? 'all-check ok' : 'all-check'} onClick={() => handleAreaAll()}>✔</button>
              <span> 모두 선택</span>
            </div>
          </div>
          <div className='type' style={{justifyContent:"flex-start"}}>
            <div><img src={grayType} className="plus-area-img"/></div>
            ...
            <div style={{margin:"13px"}}>
              <button className={allTypeCheck ? 'all-check ok' : 'all-check'} onClick={() => handleTypeAll()}>✔</button>
              <span> 모두 선택</span>
            </div>
          </div>
        ...
        <div className="plus-store-wrap">
          {storePlus && storePlus.map((store, i) => (
            <div className="store-plus" key={i} >
              <img src={store.img_url} className="store-plus-img"></img>
              <div className="store-plus-content" onClick={() => clickStoreTitle(store.id)}>
                <div>
