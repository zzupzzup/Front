      ...
      {user ? (
        <div>
          ...
          <div className="select-content">
            <div
              className={`select-word ${chatbot ? "cate" : "chatbot"}`}
              onClick={clickCate}
            >
              추천 결과 보기
            </div>
            <div
              className={`select-word ${chatbot ? "chatbot" : "cate"}`}
              onClick={clickChatbot}
            >
              챗봇에게 물어보기
            </div>
            <div
              className={`select ${chatbot ? "cate" : "chatbot"}`}
              onClick={clickCate}
            ></div>
            <div
              className={`select ${chatbot ? "chatbot" : "cate"}`}
              onClick={clickChatbot}
            ></div>
          </div>
          <button
            className="filter_btn"
            onClick={() => setSelectModalOn(true)}
          >
            <img
              style={{ width: "10px" }}
              src="https://w7.pngwing.com/pngs/550/928/png-transparent-computer-icons-iconfinder-symbol-apple-icon-format-filter-icon-angle-text-computer.png"
            ></img>
            필터
          </button>
          ...
          <div className="modal">
            <div className="modal-content">
              <div>
                <img
                  src={redArea}
                  className="modal-area-img"
                />
              </div>
              {selArea ? <div> {selArea}</div> : <div> 모든 지역을 보여드릴게요!</div>}
            </div>
            <div className="modal-content">
              <div>
                <img
          ...
          {chatbot ? <Chat chatbot={chatbot}></Chat> : <Check chatbot={chatbot}></Check>}
        </div>
      ) : (
        <div className="please-login">
          <img
            src={logo}
            className="please-img"
          />
          <img
            src={lock}
            className="lock-img"
